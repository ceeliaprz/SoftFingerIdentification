datamocap = readmatrix("validationdata/transformadas/model7elucleaned_brutos_trans_encoder_dedos_ajust.xlsx");%INCLUIR EL TIEMPO
dataencoder = readmatrix("validationdata/encoder/data_model7elucleaned_ENCODER.xlsx"); 
...
offset_time = 2+46.01;%modificar segun veamos en la grafica para que empiecen a la vez
[mocap_time,mocap_data,encoder_time,encoder_data]=test(mocap_data,offset_time,encoder_data);% los tiempos del encoder y mocap son los mismos al estar resampleados a 0.01
%% 
...
%model4elu
% encoder_refs = [0,0; 2,-0.0268; 4.05,-0.2257; 6.11, -0.2155; 8.1,-0.199; 10.01, -0.1767; 12.1,-0.148;14.17,-0.115; 16.23,-0.0782;18.18,-0.039;22.17,0.039;27.95,0;30.05,-0.43;32.08,-0.427;38.03,-0.38;54.37,0;65.99,-0.55; 82.45,0;86.03,-0.73;92.19,-0.694;96.22,-0.5;100.04,-0.388;112,0;116.02,-0.88;126.02,-0.61;138.03,0.43]; % Tiempo y valor en el encoder
% mocap_refs = [0,0;2.5,0.0957; 4.72,0.093; 6.75, 0.0877; 8.45,0.077; 10.5,0.064;13.6,0.047;16.6,0.0288;19.3,0.011;21.5,-0.0084;25.5,-0.032;31.4,-0.00423;33.6,0.1927;36.3,0.1897;44.16,0.159;60.3,-0.0059;73,0.227;89.4,-0.0054;92.9,0.339;99.7,0.2889;105,0.19;108.43,0.1255;120.3,0;124.2,0.4;135.7,0.216;147.6,-0.14]; % Tiempo y valor en el mocap

%
encoder_refs = [0,0;4.02,0;6,0;9.84,0;26.23,0;34.01,0;44.02,0;54.2,0;68.02,0;86.09,0;110.51,0;122.02,0;138.44,0]; % Tiempo y valor en el encoder
mocap_refs = [0,0;4.52,0;6.8,0;13.6,0;29.6,0;38.5,0;50.1,0;60.8,0;74.9,0;97,0;122.7,0;135.0,0;151.5,0]; % Tiempo y valor en el mocap
...
mocap_labels = {'mocap rotation X', 'mocap rotation Y', 'mocap rotation Z', 'mocap position X', 'mocap position Y', 'mocap position Z'};
 ...
 % figure;
 % hold on;
% %Estos bucles de plot para asignar las labels a cada una de las columnas
...
% figure;
% hold on;
% plot(encoder_time, encoder_data, 'DisplayName', 'encoder');
...
writetable(data_table, 'validationdata/model7elucleaned_ALINEADOS.xlsx'); % Guarda los datos en un archivo Excel
